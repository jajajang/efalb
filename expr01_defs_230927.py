...
from new_lowrank_231016 import *
...
ParamBMOracle_ = namedtuple("ParamBMOracle_", "multiplier") ##########################################
class ParamBMOracle(ParamBMOracle_):
    def shortstr(self):
        return "m=%.2g" % (self.multiplier)

ParamLowPopArt_ = namedtuple("ParamLowPopArt_", "C_T1, multiplier") ####################################
class ParamLowPopArt(ParamLowPopArt_):
    def shortstr(self):
        return "C=%.2g,m=%.2g" % (self.C_T1, self.multiplier)


ParamLowESTR_ = namedtuple("ParamLowESTR_", "C_T1, multiplier") ####################################
class ParamLowESTR(ParamLowESTR_):
    def shortstr(self):
        return "C=%.2g,m=%.2g" % (self.C_T1, self.multiplier)
...
def paramGetList2(algoName):
    paramGrid = OrderedDict()
# 	baseLambdaGrid = 10.0**np.array([-2,-1,0,1,2])
# 	baseLambdaGridExtended = 10.0**np.array([-4,-3,-2,-1,0,1,2])
    baseMultiplierGrid = 10.0**np.array([0])
    baseCT1Grid = 10.0**np.array([0.0])
    ...
    if   algoName == "bloful":
        ...
        paramGrid['multiplier'] = baseMultiplierGrid
        paramclass = ParamBMOracle
    elif algoName == "LowPopArt":  ##########################################################
        paramGrid['C_T1'] = baseCT1Grid
        paramGrid['multiplier'] = baseMultiplierGrid
        paramclass = ParamLowPopArt
    elif algoName == "LowESTR":  ##########################################################
        paramGrid['C_T1'] = baseCT1Grid
        paramGrid['multiplier'] = baseMultiplierGrid
        paramclass = ParamLowESTR
    elif algoName == "EFALB":
        paramGrid['multiplier'] = baseMultiplierGrid
        paramclass = ParamEFALB
    elif algoName.startswith("bltwostage"):
...
def banditFactory(data, algoName, algoParam, exprOpts):
    ...
    if algoName == "bloful":
        ...
        Sp = np.sqrt(lam)*data.S_F
        algo = BMOracle(data.X, data.Z, lam, data.r, exprOpts.T, data.cheatU, data.cheatV, data.R, Sp,
                            multiplier=algoParam.multiplier)
        ...
        svals = sla.svdvals(data.Th)[:data.r]
        algo = LowPopArt_rankone(data.X, data.Z, 
                                lam, data.R,
                                data.S_F,
                                svals.max(), svals.min(), data.r, 
                                algoParam.C_T1, exprOpts.T,
