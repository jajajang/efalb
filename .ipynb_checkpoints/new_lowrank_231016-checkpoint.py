...
def experimental_design_F1(A,d1,d2):
        N_a, d = np.shape(A)
        mu=cvx.Variable(N_a, pos=True)
        X=A.T@ cvx.diag(mu) @A
        
        T = cvx.Variable((d, d), symmetric=True)
        M = cvx.bmat([[X, np.eye(d)],
                  [np.eye(d), T]])
        constraints = [M >> 0, cvx.sum(mu)==1]
        Tsum=np.zeros([d2,d2])
        for i in range(0,d1):
            Tsum=Tsum+T[d2*i:d2*(i+1), :][:,d2*i:d2*(i+1)]
        objective = cvx.Minimize(cvx.lambda_max(Tsum))
        ...
        Q_inv=la.inv(Q)
        return dist_our, NN, Q



def experimental_design_Cmin(A,d1,d2):
    N_a, d = np.shape(A)
    mu=cvx.Variable(N_a, pos=True)
    X=A.T@ cvx.diag(mu) @A
    
    constraints = [cvx.sum(mu)==1]
    prob = cvx.Problem(cvx.Maximize(cvx.lambda_min(X)), constraints)
    prob.solve(solver=cvx.MOSEK, verbose=True)
    
    dist=mu.value/np.sum(mu.value)
    Cmin=1/prob.value
    
    Q=A.T@ np.diag(dist) @A
    Q_inv=la.inv(Q)
    eigvals, eigvect = la.eig(Q)
    eig_max=np.max(eigvals)
    return dist, Cmin, Q_inv, eig_max
...
def loss_fn(X, Y, beta):
    return cvx.norm2(X @ beta - Y) ** 2

def regularizer(beta,d1,d2):
    return cvx.norm(cvx.reshape(beta, (d1,d2)), 'nuc')
...
#            self.W_invVt_norm_sq -= (np.dot(self.W, tempval1) ** 2) / (1 + tempval2) # efficient update, O(Nd)
        ...
        if self.t<1000 or (self.t%100==0):
            U, V, out_nIter, stat = rankone(self.myX, self.myZ, self.myRewards, self.r, self.R)
...
########################################
    ...
    def next_arm(self):
        if (self.t == 1):
            ...
            minT1= self.invX_norm*self.invZ_norm*(self.sval_max**2)/(self.sval_min**3)*self.R*(np.max([self.d1,self.d2])**(3/2))*np.sqrt(self.r*self.T)
            self.T1 = int(np.ceil(self.C_T1 * minT1))
...
########################################
    """ this is rank-one version of LowPopArt. The only difference is the action set.
    """
    def __init__(self, X, Z, lam, R, S_0, sval_max, sval_min, r, C_T1, T, flags={}, subsample_func=None, subsample_rate=1.0, multiplier=1.0, binaryRewards=False, bArmRemoval=False, SpType=None):
        """
        Two stage: internally uses BilinearOful object.
        """
        self.X = X
        ...
        self.stage1_dist, self.action_set_geom_value, self.Q_inv = experimental_design_F1(self.W, self.d1, self.d2)


        
        #- new scheme
        minT1 = np.sqrt(np.sqrt(np.max([self.d1, self.d2]))*self.T*self.action_set_geom_value)/self.sval_min
        self.T1 = int(np.ceil(C_T1 * minT1))

        
        self.stage1arms = np.zeros((self.T1,2))
        self.stage1rewards = np.zeros(self.T1)
        
        self.X_hist=np.zeros((self.T1, self.d)) # First stage one-step estimator history
    ...
    def next_arm(self):
        if (self.t <= self.T1):
            ...
            action_index=np.random.choice(self.N, p=self.stage1_dist)
            armPairToPull = np.unravel_index(action_index, (self.N1, self.N2))
            radius_sq = np.nan
        else:
            # at the beginning of the second stage
            if (self.t == self.T1+1):
                #----- invoke matrix completion
                self.vari = (self.S_nuc**2 + self.R**2)*self.action_set_geom_value
                
                threshold = np.sqrt(2*self.action_set_geom_value*np.log((self.d1+self.d2)/self.delta)/self.T1)
                nu_our = np.sqrt(2*np.log(2*np.max((self.d1,self.d2))/self.delta)/self.action_set_geom_value/self.T1)/(self.S_nuc+self.R)

                theta_hat_raw=matrix_catoni(self.X_hist,self.T1, self.d1, self.d2,nu_our)
                #- get the subspaces
                [self.hatUFull, self.hatSFull, VT] = thresholding(theta_hat_raw, threshold)
                self.hatVFull = VT.T
                ...
                #- prepare oful
                d = np.max([self.d1, self.d2]) # FIXME just an impromptu
                T2 = self.T - self.T1
                self.lamp = T2/d/self.r/np.log(1+T2/self.lam) # FIXME I think I should use T rather than T2...
                term1 = np.sqrt(self.lam) * self.S_nuc #2 * np.sqrt(d*self.r)
                ...
                #######################################
                term2 = self.action_set_geom_value* (self.R **2) / self.T1/(self.sval_min**2)
                B_perp = term2*self.sval_max
...
#                self.D = np.diag([self.lam]*k + [self.lamp]*(p-k))
                ...
                #- pseudo play oful, so it is up to date!
                for myt in range(self.T1):
                    self.oful.update( np.array(self.stage1arms[myt,:], dtype=int), self.stage1rewards[myt] )
    ...
    def update(self, pulled_arm_pair, y_t):
        if (self.t <= self.T1):
            self.stage1arms[self.t-1]=pulled_arm_pair
            self.stage1rewards[self.t-1]=y_t
            act_t=np.outer(self.X[pulled_arm_pair[0],:], self.Z[pulled_arm_pair[1],:]).ravel()
            self.X_hist[self.t-1]=(y_t-act_t@self.theta_0)*(self.Q_inv@act_t)+self.theta_0
            self.theta_hat=self.X_hist[self.t-1] # Temporary placeholder for theta_hat. One-step version
        else:
            self.oful.update(pulled_arm_pair, y_t)
        self.t += 1
...
########################################
    ...
    def __init__(self, X, Z, lam, R, S_0, sval_max, sval_min, r, C_T1, T, flags={}, subsample_func=None, subsample_rate=1.0, multiplier=1.0, binaryRewards=False, bArmRemoval=False, SpType=None):
        ...
        self.stage1_dist, self.action_set_geom_value, self.Q_inv, self.Q_eigmax = experimental_design_Cmin(self.W, self.d1, self.d2)


        
        #- new scheme
        minT1 = np.sqrt(self.r*(np.max((self.d1, self.d2))**(3/2))*self.Q_eigmax*self.T*(self.action_set_geom_value**2))*self.S_nuc/self.sval_min
        self.T1 = int(np.ceil(C_T1 * minT1))
        ...
        self.stage1hist=np.zeros((self.T1, self.d)) # First stage one-step estimator history
    ...
    def next_arm(self):
        if (self.t <= self.T1):
            ...
            if (self.t == self.T1+1):
                #----- invoke matrix completion
                theta_var = cvx.Variable(self.d)
                lambd_b = (self.R+self.S_nuc)*np.sqrt(np.log(self.d1+self.d2) * self.T1)
                lassosol = cvx.Problem(cvx.Minimize(objective_fn(self.stage1hist, self.stage1rewards, theta_var, lambd_b, self.d1, self.d2)))
                lassosol.solve()
                theta_hat = theta_var.value
                #- get the subspaces
                [self.hatUFull, self.hatSFull, VT] = la.svd(np.reshape(theta_hat,(self.d1, self.d2)))
                self.hatVFull = VT.T
                ...
                #######################################
                term2 = d*self.Q_eigmax*(self.action_set_geom_value**2)* (self.R **2)*self.r*(self.S_nuc**2) / self.T1/(self.sval_min**2)
                B_perp = term2*self.sval_max
...
#                self.D = np.diag([self.lam]*k + [self.lamp]*(p-k))
    ...
    def update(self, pulled_arm_pair, y_t):
        if (self.t <= self.T1):
            ...
            self.stage1rewards[self.t-1]=y_t
            act_t=np.outer(self.X[pulled_arm_pair[0],:], self.Z[pulled_arm_pair[1],:]).ravel()
            self.stage1hist[self.t-1]=act_t
        else:
            self.oful.update(pulled_arm_pair, y_t)
        self.t += 1
