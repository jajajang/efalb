...
def rankone_modif(X,Z,y,r,cheatU, cheatV, R=.1, C=.1, tolPred=0.01, tolTh=0.01, maxIter=400, verbose=False):
    """
    matrix recovery with rank-one measurements using Burer-Monteiro approach
    measurement model: (X[i,:] @ Theta) @ Z[i,:] == y[i]
    (IN)
    ...
    N,d = X.shape
    U = cheatU
    V = cheatV # just a placeholder
    M = np.zeros( (d*r,d*r) )
    hatTh = cheatU @ cheatV.T # very bad initial hatTh
    if (verbose):
        my_tqdm = tqdm
    else:
        my_tqdm = lambda x: x
